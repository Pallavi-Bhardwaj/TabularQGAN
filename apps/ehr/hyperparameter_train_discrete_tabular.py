...
import hashlib
import math 
from qugen.main.data.helper import tabular_feature_names, metrics_report, run_external_classical_tabular

from qugen.main.generator.internal_classical_tabular_gan import run_classical_tabular
...
# classical_model_size  = 'large' 
# random_seed           = 8
# root_path = f'{os.getcwd()}/apps/ehr' 
...
# write to csv to show that that experiment ran
total_run_log_path ='run_log.csv'

#for quantum model what is the metric which picks the best epoch  
decision_metric = 'overall_metric'

with open(total_run_log_path, "a") as file:
...
if model_type != 'qgan':
    ...
    if model_type == 'icgan':
        # classical_data_name = data_set_name.replace('_non_boolean', '')s
        # classical_gan_data = pd.read_csv(f"training_data/{classical_data_name}_classical_data.csv")
        synthetic_data, best_epoch, icgan_score = run_classical_tabular(human_readable_data, data_spec, n_epochs, lr_generator, lr_discriminator,
                                                   batch_size, numerical_features, cat_features, input_size, num_hidden_layers_gen,
                                                    num_dimensions_gen, num_hidden_layers_disc, num_dimensions_disc, plots_path)
...
if model_type != "qgan":
    ...
    with open(f'{plots_path}meta.json', "w") as file:
          file.write(str(meta_dict))

# save to arguments to run log file if run successful 
with open(total_run_log_path, "a") as file:
    current_time =datetime.now().strftime('%Y-%m-%d %H:%M')
    file.write(f'{current_time},')
    file.write(str(sys.argv))
