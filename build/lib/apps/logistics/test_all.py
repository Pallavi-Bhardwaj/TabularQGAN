...
from qugen.main.generator.discrete_qcbm_model_handler_JAX import (
    DiscreteQCBMModelHandlerJAX,
)
from qugen.main.generator.discrete_qgan_model_handler_JAX import (
    DiscreteQGANModelHandlerJAX,
)
...
minimum_kl_data_path = f"../../../QML-QGAN-Analytics/data"
...
def helper_test_configuration(
    ...
    data_set_path = f"./training_data/{data_set_name}_{dimension}D"
...
def test_discrete_qcbm(conf):
    ...
    if fixed_hyperparameters["hot_starting"]:
        for hotstart_depth in range(1, circuit_depth + 1):
            ...
            model = DiscreteQCBMModelHandlerJAX()
            model.build(
                name,
                data_set_name,
...
def test_discrete_qgan(conf):
    ...
    model = DiscreteQGANModelHandlerJAX()
