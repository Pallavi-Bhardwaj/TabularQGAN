...
from qugen.main.data.data_handler import PITNormalizer, MinMaxNormalizer, TabularDataTransformer
from qugen.main.data.helper import kl_divergence, kl_divergence_from_data_tabular, get_metadata, evaluate_quality
...
class DiscreteQGANModelHandler(BaseModelHandler):
    ...
    def build(
        ...
        if self.circuit_type == 'copula' and self.transformation != 'pit':
            raise ValueError(
                "Copula circuit must have PIT transformation. Current transformation: " + self.transformation)
        ...
        elif self.transformation == 'tabular':
             self.normalizer = TabularDataTransformer(data_spec)

        else:
            raise ValueError("Transformation value must be either 'minmax' , 'tabular' or 'pit'")
        ...
        if self.circuit_type == 'copula':
            ...
            from qugen.main.generator.quantum_circuits.discrete_generator_pennylane import discrete_standard_circuit_JAX as get_generator
        elif self.circuit_type == 'tabular':
        ...
        if self.circuit_type != "tabular":
            self.generator, self.num_generator_params, dummy_qnode  = get_generator(self.n_qubits, self.n_registers , self.circuit_depth)
        else:
        ...
        # self.generator_weights = jnp.zeros((self.num_generator_params,)) # TODO: Delete after verification
        ...
        # print ('qml spec ' ,qml.specs(self.generator)(subkey, self.generator_weights, 1))
        fig, ax = qml.draw_mpl(dummy_qnode)(jnp.zeros((self.num_generator_params,)))
         # fig.savefig(f"tabular_circuit_given_rotation={add_given_rotation}_cond_rotations={entangle_cat_numerical}.png")
        # fig, ax = qml.draw_mpl(self.generator)(subkey, self.generator_weights, 1)
    ...
    def reload(
        ...
        if self.generator is None:
            if self.circuit_type == 'copula':
                from qugen.main.generator.quantum_circuits.discrete_generator_pennylane import discrete_copula_circuit_JAX as get_generator
            elif self.circuit_type == 'standard':
                from qugen.main.generator.quantum_circuits.discrete_generator_pennylane import discrete_standard_circuit_JAX as get_generator
            elif self.circuit_type == 'tabular':
                from qugen.main.generator.quantum_circuits.discrete_generator_pennylane import discrete_tabular_circuit_JAX as get_generator
            ...
            if self.circuit_type != "tabular":
                self.generator, self.num_generator_params, _  = get_generator(self.n_qubits, self.n_registers, self.circuit_depth)
            else:
    ...
    def train(
        ...
        if not self.circuit_type == 'tabular':
